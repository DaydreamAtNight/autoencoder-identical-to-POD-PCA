...
class AE(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        _,S,V = torch.svd(self.decoder[0].parametrizations.weight.original)
        encoded = encoded@V@torch.diag(S)
...
# torch.nn.utils.parametrizations.parametrize.register_parametrization(model.encoder[0], "weight", Svd_W_en())
torch.nn.utils.parametrizations.parametrize.register_parametrization(model.decoder[0], "weight", Svd_W_de())
# model.decoder[0].weight.requires_grad = False # freeze the decoder
# model.encoder[0].weight = nn.Parameter(torch.tensor(ortho_group.rvs(dim=784)[:,:81].T).float())
# model.decoder[0].weight = nn.Parameter(torch.tensor(ortho_group.rvs(dim=784)[:,:81]).float())
...
for epoch in range(epochs):
    ...
    for step, image in enumerate(train_loader):
        ...
        # loss_ortho_de = torch.dist(model.decoder[0].weight.T @ model.decoder[0].weight, torch.eye(81).to(device))
        # loss_ortho_en = torch.square(torch.dist(model.encoder[0].weight @ model.encoder[0].weight.T, torch.eye(81).to(device)))
