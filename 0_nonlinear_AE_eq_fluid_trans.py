...
class AE(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = torch.nn.Sequential(
            torch.nn.Linear(flatten_data.shape[1], 2*k, bias=False),
            torch.nn.Tanh(),
            torch.nn.Linear(2*k, k, bias=False),
            torch.nn.Tanh(),
            )
        self.decoder = torch.nn.Sequential(
            torch.nn.Linear(k, 2*k, bias=False),
            torch.nn.Tanh(),
            torch.nn.Linear(2*k, flatten_data.shape[1], bias=False),
            torch.nn.Tanh(),
            )
